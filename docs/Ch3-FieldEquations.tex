...
\author{%
  Romeo Matshaba\\[4pt]
  \normalsize Department of Physics, University of South Africa (UNISA)\\[2pt]
...
matching the exact Schwarzschild component.  The full $g_{rr} = 1/f$ with $f=1-\rs/r$
requires either a radial source $\sigma_r \ne 0$ or the Kerr--Schild sector (see
Section~\ref{sec:Gmu}).
...
\textbf{Critical distinction from GR:} In General Relativity one varies $S$ with respect
...
This is the term most often missed in approximate treatments.  By Jacobi's formula:
\begin{equation}
...
\begin{remark}
Equation~\eqref{eq:compact} has no analogue in General Relativity.  In GR the field
equation for $g_{\mu\nu}$ is algebraic in second derivatives; here the wave operator
$\nabla^2$ acting on $\sigma$ appears naturally from the kinetic sector, producing a
genuine propagating field theory for gravity.
\end{remark}
...
The key structural result connecting the Einstein tensor to the wave operator is:
...
$Q_\mu$ is the direct analogue of the Yang--Mills gluon self-coupling $Q \sim A\cdot F$.
Gravity gravitates: the graviton field's own energy density curves the spacetime through
which it propagates.  At cosmological scales, $Q_\mu$ generates an additional attractive
potential well that produces dark-matter-like phenomenology without weakly interacting
massive particles.
...
The origin of $\Delta G_t$ is precisely the geometric pressure from channel~(J):
the Jacobi formula contribution~\eqref{eq:jac_val} generates an additional source
\[
  G_t^{(\mathrm{pressure})} \equiv
  -\frac{\sigt}{f}\cdot\frac{(\nabla_r\sigt)^2}{g_{rr}}
\]
that is not captured by the Kerr--Schild directional derivatives~\eqref{eq:G1}--\eqref{eq:G3}.
Including it closes the gap: $G_t^{(\mathrm{lit})} + G_t^{(\mathrm{pressure})} = G_t^{(\mathrm{req})}$.
...
\paragraph{Physical meaning.}
$T_\mu$ is the mathematical realisation of the equivalence principle: matter does not
``curve'' spacetime directly; it modifies the phase of the $\sigma$-field, and it is
this phase modification that we perceive as curvature.  Every form of energy with
$T^{\mu\nu} \ne 0$ sources graviton waves universally.

\subsection{Term 4: Quantum stiffness $\kappa\lQ^2\boxg^2\sigma_\mu$}
\label{sec:stiffness}

The fourth-order term arises from the higher-derivative part of the kinetic action.
It is the QGD mechanism for singularity avoidance:
...
The repulsive quantum stiffness therefore \emph{dominates} at $r \lesssim \lQ$,
providing a physical cut-off with no free parameters.  This makes QGD a
self-regulating system.

\paragraph{Scaling.}
At astrophysical scales $r \gg \lQ \approx 10^{-35}$~m, the stiffness term is
suppressed by $\kappa\lQ^2/r^2 \ll 1$ and completely negligible.
...
\begin{proof}
The four components are computed directly.  Using the Schwarzschild Christoffel symbols
$\Gamma^t_{tr} = \rs/(2r^2f)$, $\Gamma^r_{tt} = f\rs/(2r^2)$,
$\Gamma^r_{rr} = -\rs/(2r^2f)$, $\Gamma^r_{\theta\theta} = -rf$,
and the first covariant derivatives $\nabla_r\sigt = -\sigt/(2rf)$:
\begin{align*}
  ...
  &= -\frac{\sqrt{\rs}}{2r^{5/2}}.
\end{align*}
Summing gives $\boxg\sigt = -\sqrt{\rs}(r-\rs)/(4r^{7/2})$.  The equivalence to
$\sigt(\sigt^2-1)/(4r^2)$ follows from $\sigt^2 = \rs/r$ and $f = 1-\rs/r$.
\end{proof}
...
microscopic Dirac theory: $V(\sigt)$ emerges from spontaneous chiral symmetry breaking
with $\sigt$ as the order parameter.  This gives a Lagrangian path to closure with
no free parameters.
...
\begin{equation}
  ...
  \label{eq:gap}
\end{equation}
which $G_t$ (geometric coupling, including the geometric pressure) must supply.
...
\begin{equation}
  ...
  \label{eq:quadrupole}
\end{equation}
matching the standard quadrupole formula, derived purely from $\sigma$-field
superposition without any additional postulates.

\subsection{Gauge-field duality}

A key insight is the separation of physical and coordinate-dependent components:
...
$g_{rr} = 1/f$ corresponds to a different gauge choice with $\sigma_r \ne 0$.  The
perceived spatial curvature of the radial metric is a gauge artefact; the physics
is entirely encoded in $\sigma_t$.
...
\begin{equation}
  G_{\mathrm{QGD}}(x,x') = \kappa\lQ^2\bigl[G_0(x,x') - G_{\mQ}(x,x')\bigr]
  \label{eq:GQGD}
\end{equation}
satisfying $\mathcal{D}_x G_{\mathrm{QGD}}(x,x') = \delta^4(x-x')/\sqrt{-g(x)}$.
\end{definition}

The two sectors:
\begin{itemize}[nosep]
  \item $G_0$ (massless): light-cone propagation, $1/r$ Newtonian potential, exact GR.
  \item $-G_{\mQ}$ (massive): Yukawa-suppressed $e^{-\mQ r}/r$ on length scale $\lQ$;
    resolves singularities; localises quantum corrections.
\end{itemize}
...
\begin{equation}
  \boxed{\sigma_\mu(x) = \sigma_\mu^{\mathrm{free}}(x)
    + \kappa\lQ^2\!\int_{\mathcal{M}}\!\dd^4x'\sqrt{-g(x')}\,
...
UV-finite integrals at each order.
...
$\boxg(\boxg-\mQ^2)\sigma_\mu = -\mQ^2 S_\mu$ &
  Pais--Uhlenbeck form \\
$\sigma_\mu = \sigma^{\mathrm{free}} + \kappa\lQ^2\!\int(G_0-G_{\mQ})S_\mu$ &
  Exact Green's function solution \\
$\mathcal{G}[\mathcal{M}(\sigma)] = \mathcal{J}\cdot\mathcal{Q}[\sigma]$ &
...
\begin{itemize}[nosep]
  ...
  \item The Schwarzschild $\sigma$-field satisfies $\boxg\sigt = \sigt(\sigt^2-1)/(4r^2)$
    (Theorem~\ref{thm:box}), verified symbolically with zero residual.
  \item The exact required $G_t$ is determined by the vacuum constraint; the gap
    from the literature formula is identified and its origin (geometric pressure)
...
\begin{itemize}[nosep]
  ...
  \item Derivation of the $g_{rr} = 1/f$ component from a consistent $\sigma_r$
    source with definite signature.
  \item Systematic computation of the PN hierarchy beyond 1PN from the two-body
    interaction equation~\eqref{eq:interaction}.
\end{itemize}
