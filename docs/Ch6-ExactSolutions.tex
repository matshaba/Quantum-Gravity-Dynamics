...
\begin{equation}\documentclass[12pt,a4paper]{article}
...
\fancyhead[R]{\small\textit{Chapter 6}}
...
\begin{titlepage}
  ...
  {\Huge\bfseries Chapter 6}\\[0.5em]
  {\Large\bfseries Post-Newtonian Hierarchy, N-Body Exact Solutions,\\[0.25em]
   and the QGD Master Formula}\\[0.6em]
...
This chapter derives the post-Newtonian (PN) expansion of binary dynamics from the QGD action, establishes the exact N-body metric and equations of motion, and presents the QGD master formula generating all test-body binding-energy coefficients as exact rational numbers.

The gravitational wave flux in QGD consists of a single physical sector at all astrophysical separations: the spin-2 tensor (quadrupole) radiation. The spin-0 sector of the $\sigma$-field acquires a Planck-scale effective mass in the vacuum and is Yukawa-suppressed over any macroscopic distance by a factor $e^{-r/\lP}$. The physical consequences of this suppression are detailed in Chapter~13. Here we present the PN hierarchy in its corrected form, consistent with the mode decomposition established there.

% -----------------------------------------------------------------------
\section{N-Body Exact Solutions}

\subsection{Single Kerr Body}

For a spinning body of mass $M_a$ with spin parameter $\alpha_a = J_a/(M_a c)$ and
$\mathcal{S}_a = r_a^2 + \alpha_a^2\cos^2\theta_a$, the graviton field components are:
\begin{equation}
  \sigma_t^{(a)} = \sqrt{\frac{2GM_a\,r_a}{c^2\,\mathcal{S}_a}},
  \qquad
  \sigma_\phi^{(a)} = \alpha_a\sin\theta_a\sqrt{\frac{2GM_a}{c^2\,r_a\,\mathcal{S}_a}}.
\end{equation}
For Schwarzschild ($\alpha_a = 0$): $\sigma_t^{(a)} = \sqrt{2GM_a/(c^2 r_a)}$, $\sigma_\phi^{(a)} = 0$.

\subsection{Two-Body Metric (Exact, Equatorial Plane)}

The exact metric for two bodies follows by superposition of their $\sigma$-fields:
\begin{align}
  g_{tt} &= -\bigl[1 - (\sigma_t^{(1)} + \sigma_t^{(2)})^2\bigr], \\
  g_{t\phi} &= -r\sin\theta\,(\sigma_t^{(1)} + \sigma_t^{(2)})(\sigma_\phi^{(1)} + \sigma_\phi^{(2)}), \\
  g_{\phi\phi} &= r^2\sin^2\theta\bigl[1 + (\sigma_\phi^{(1)} + \sigma_\phi^{(2)})^2\bigr].
\end{align}
Expanding the time-time component reveals three distinct contributions:
\begin{equation}
  \boxed{g_{tt} = -\left[1
    - \underbrace{(\sigma_t^{(1)})^2}_{2GM_1/(c^2r_1)}
    - \underbrace{(\sigma_t^{(2)})^2}_{2GM_2/(c^2r_2)}
    - \underbrace{2\sigma_t^{(1)}\sigma_t^{(2)}}_{\text{QGD cross-term }\sim r^{-1}}
  \right].}
\end{equation}
The self-terms reproduce the individual Schwarzschild potentials. The cross-term
$2\sigma_t^{(1)}\sigma_t^{(2)} \propto \sqrt{M_1M_2}/r$ falls as $r^{-1}$ and generates
the Newtonian binding energy when integrated.

\subsection{N-Body Metric}

For $N$ bodies the metric generalises to:
\begin{align}
  g_{tt} &= -\left[1 - \sum_{a}A_a^2 - 2\sum_{a<b}A_aA_b\right], \\
  g_{t\phi} &= -r\sin\theta\left[\sum_a A_a B_a + \sum_{a\neq b}A_a B_b\right],
\end{align}
where $A_a = \sigma_t^{(a)}$, $B_a = \sigma_\phi^{(a)}$. Computational complexity is
$\mathcal{O}(N)$ per field point, versus $\mathcal{O}(N_\mathrm{grid}^3\times N_\mathrm{steps})$
for numerical GR.

\subsection{Superposition Theorem}

\begin{theorem}[Rigorous superposition error bound]
\begin{equation}
  \frac{|\sigma_t^{(\mathrm{tot})} - \sum_a\sigma_t^{(a)}|}{\sum_a\sigma_t^{(a)}}
...
\begin{equation}
  \boxed{d_\mathrm{merge} = \frac{2GM_1}{c^2}\left(1 + \left(\frac{M_2}{M_1}\right)^{1/3}\right)^3.}
\end{equation}
For equal masses: $d_\mathrm{merge} = 16Gm/c^2 = 4r_s^\mathrm{(total)}$.
Verified: $\Sigma_\mathrm{saddle} = 1.000000$ at all mass ratios.

\subsection{Equations of Motion}

\begin{equation}
  \boxed{\ddot{\mathbf{x}} =
    \underbrace{-\frac{GM_1}{r_1^2}\hat{r}_1 - \frac{GM_2}{r_2^2}\hat{r}_2}_{\text{Newtonian}}
    + G\sqrt{M_1M_2}\left[\frac{\hat{r}_2}{\sqrt{r_1}\,r_2^{3/2}}
      + \frac{\hat{r}_1}{\sqrt{r_2}\,r_1^{3/2}}\right]_{\!\!\text{QGD cross.}}}
\end{equation}
The cross-force scales as $r^{-3/2}$ (softer than Newton). The mutual two-body force is
exact Newton (verified numerically).
...
\begin{equation}
  M = M_1 + M_2, \quad
  \mu = \frac{M_1 M_2}{M}, \quad
  \eta = \frac{M_1 M_2}{M^2}, \quad
  x = \left(\frac{GM\Omega}{c^3}\right)^{2/3},
\end{equation}
where $\Omega$ is the orbital angular frequency and $x$ is the standard PN parameter.
The dipole asymmetry factor:
\begin{equation}
  D = \frac{(\sqrt{M_1} - \sqrt{M_2})^2}{M},
\end{equation}
characterises the degree of mass asymmetry in the $\sigma$-field amplitude and governs
the near-field energy structure of the binary. Its role in observable radiation is discussed
in Section~\ref{sec:flux}.

% -----------------------------------------------------------------------
\section{Binding Energy}

The PN expansion of the binding energy:
\begin{equation}
  E(x) = -\frac{\mu c^2}{2}x\left\{1 + \sum_{n=1}^4 e_n x^n + \mathcal{O}(x^5)\right\},
\end{equation}
with coefficients:
\begin{align}
  ...
  e_3 &= -\frac{675}{64} + \left(\frac{34445}{576} - \frac{205\pi^2}{96}\right)\eta
         - \frac{155}{96}\eta^2 - \frac{35}{5184}\eta^3.
\end{align}

% -----------------------------------------------------------------------
\section{QGD Master Formula}

The exact Schwarzschild binding energy for circular test-body geodesics:
\begin{equation}
  \frac{E_\mathrm{bind}}{\mu} = \frac{1-2x}{\sqrt{1-3x}} - 1.
\end{equation}
Taylor-expanding in $x$ yields the \textbf{master formula} generating all $\eta=0$
PN coefficients as exact rational numbers:
\begin{equation}
  \boxed{e_n^{(\eta=0)} = -\binom{2n}{n}\left(\frac{3}{4}\right)^n\frac{2n-1}{n+1}.}
\end{equation}
...
3 & $-675/64$ & GR known\\
4 & $-3969/128$ & GR known\\
5 & $-45927/512$ & \textbf{QGD prediction}\\
6 & $-264627/1024$ & \textbf{QGD prediction}\\
7 & $-12196899/16384$ & \textbf{QGD prediction}\\
8 & $-70366725/32768$ & \textbf{QGD prediction}\\
\bottomrule
\end{tabular}
\end{center}

\subsection{Spinning Series}

\textbf{Spin-orbit coefficients} (entering at $\chi\,u^{n+5/2}$):
\begin{equation}
  \boxed{e_n^\mathrm{SO} = -\frac{(2n+1)!!\,3^n}{2^n\,n!}.}
\end{equation}

\textbf{Spin-squared coefficients} (entering at $\chi^2 u^{n+3}$):
\begin{equation}
  \boxed{e_n^\mathrm{SS} = \frac{(2n+3)!!\,3^n}{3\cdot2^{n+1}\,n!}.}
\end{equation}

% -----------------------------------------------------------------------
\section{Gravitational Wave Flux}
\label{sec:flux}

\subsection{GR Quadrupole Flux (Observable)}

The observable gravitational wave flux carried by the massless spin-2 tensor modes is:
\begin{equation}
  \boxed{\Fcal(x) = \frac{32}{5}\frac{c^5}{G}\eta^2 x^5
    \left\{1 + f_1 x + f_{3/2} x^{3/2} + f_2 x^2 + \cdots\right\},}
\end{equation}
with:
\begin{equation}
  f_1 = -\frac{1247}{336} - \frac{35}{12}\eta,
  \qquad f_{3/2} = 4\pi\;\text{(tail)}.
\end{equation}
This is identical to the GR result because the QGD tensor sector and GR linearised gravity
are the same theory (Chapter~13, Theorem~1).

\subsection{The Near-Field $\sigma$-Cross-Term}

The cross-field product $\sigma_1\sigma_2$ in the binary's near zone has the
multipole structure:
\begin{equation}
  \sigma_1\sigma_2 \approx \frac{2G\sqrt{M_1M_2}}{c^2\,r}
  \left[1 + \frac{\mathbf{x}\cdot(\mathbf{r}_1+\mathbf{r}_2)}{2r^2} + \cdots\right],
\end{equation}
where $\mathbf{r}_a$ are positions relative to the centre of mass.
The dipole coefficient is $\mathbf{r}_1 + \mathbf{r}_2 = ((M_1-M_2)/M)\,\mathbf{r}_{12}$,
which is non-zero for unequal masses.

This dipole component of $\sigma_1\sigma_2$ sources the massive spin-0 mode of the
$\sigma$-field. The effective mass of the spin-0 mode in the vacuum
($\sigma\to 0$) is $m_Q\sim m_\mathrm{Pl}/\sqrt{\kappa}$, giving a Compton wavelength
of order $\lP$. The resulting Yukawa suppression of any spin-0 radiation reaching a
detector at distance $r$ from the binary is:
\begin{equation}
  e^{-r/\lP} \approx e^{-10^{44}} \quad \text{(HT B1913+16)},
\end{equation}
which is effectively zero for any astrophysical source or detector.
The near-field $\sigma$-cross-term therefore has no observable gravitational wave signature.

\subsection{The $D$-Factor and Near-Field Energy Structure}

The dipole asymmetry factor $D = (\sqrt{M_1}-\sqrt{M_2})^2/M$ characterises the amplitude
difference of the two bodies' $\sigma$-fields. It enters the near-field energy balance
through the cross-term $\sigma_t^{(1)}\sigma_t^{(2)}$, which is proportional to
$\sqrt{M_1M_2}/r$ and contributes to the conservative orbital dynamics.

$D$ does not enter the observable gravitational wave flux: the flux is determined
entirely by the spin-2 tensor modes, which see only the mass quadrupole
$Q_{ij} = \sum_a M_a x_a^i x_a^j$. The flux formula above is complete and exact
for the observable radiation.

% -----------------------------------------------------------------------
\section{TaylorF2 Waveform Phase}

\begin{equation}
  \Psi(f) = 2\pi f t_c - \varphi_c - \frac{\pi}{4}
    + \frac{3}{128\eta x^{5/2}}\left\{1 + \psi_1 x + \psi_{3/2}x^{3/2} + \cdots\right\},
\end{equation}
with:
\begin{equation}
  \psi_1 = \frac{3715}{1008} + \frac{55}{12}\eta,
  \qquad \psi_{3/2} = -10\pi.
\end{equation}
These coefficients match GR exactly at all orders computed, as required by the
tensor-sector equivalence. The $D$-factor does not contribute to the observable
GW phase at any PN order.

% -----------------------------------------------------------------------
\section{Orbital Decay: Hulse-Taylor Verification}

The orbital period decay rate from the quadrupole formula, with Peters eccentricity factor
$f(e) = (1 + \frac{73}{24}e^2 + \frac{37}{96}e^4)/(1-e^2)^{7/2}$:
\begin{equation}
  \dot{P}_b = -\frac{192\pi}{5}\left(\frac{GM}{c^3}\right)^{5/3}
    \left(\frac{2\pi}{P_b}\right)^{5/3}\frac{M_1 M_2}{M^2}\,f(e).
\end{equation}
...
$e$ & $0.6171$ & Observed\\
$\dot{P}_b^\mathrm{QGD}$ & $-2.4054\times10^{-12}$\,s/s & QGD = GR formula\\
...
The agreement is at the level of radiative corrections not yet included in the formula.
No dipole contribution is present or needed.

% -----------------------------------------------------------------------
\section{QGD vs.\ GR: Paradigm Comparison}
...
\midrule
Fundamental variable & $g_{\mu\nu}$ (10 components) & $\sigma_\mu$ (4 components)\\[3pt]
Metric status & Fundamental & Derived output\\[3pt]
Field equations & 10 coupled nonlinear PDEs & 4 linear + algebra\\[3pt]
Superposition & Impossible at metric level & Exact at $\sigma$-level\\[3pt]
Gravitational energy & Pseudotensor & True tensor $\tfrac{1}{2}(\partial\sigma)^2$\\[3pt]
Einstein equations & Postulated & Derived ($\nabla^2\sigma = 0$)\\[3pt]
Observable GW flux & Quadrupole & Quadrupole (identical)\\[3pt]
Dipole radiation & Forbidden (momentum conservation) & Forbidden (Yukawa $e^{-r/\lP}$)\\[3pt]
UV divergences & Quadratic ($\Lambda^2$) & Logarithmic ($\ln\Lambda$)\\[3pt]
