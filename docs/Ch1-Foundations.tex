...
\begin{document}
	...
	\begin{tcolorbox}[enhanced,breakable,
	  ...
	  title={\textbf{Required correction: the microscopic action must include interactions}},
	  fonttitle=\bfseries]
	Coarse-graining the \emph{free} Dirac action $S_D$ generates only mass and wavefunction
	renormalisation---no new interaction operators can emerge from a theory with none.
	The pressure term $P(\bar\psi\psi)$, Maxwell kinetic term $F^2$, and angular-momentum
	coupling $J^2\bar\psi\psi$ in the effective Lagrangian require an interacting microscopic
	starting point.  The correct microscopic action is:
	\begin{equation}
	...
	\end{equation}
	where $G_{\rm NJL}$ is the Nambu--Jona-Lasinio coupling.  Wilsonian coarse-graining of
	$\mathcal{S}_{\rm micro}$ generates $P(\bar\psi\psi)$ from the four-Fermi term,
	$-\tfrac{1}{4}F^2$ from the EM coupling, the $J^2$ operator from its derivative
	expansion, and $\rho_\Lambda$ from zero-point fluctuations.  The free action
	$S_D$~\eqref{eq:dirac_action} describes the \emph{single-particle sector} only.
	The coarse-graining results in the subsequent sections are understood as derived
	from $\mathcal{S}_{\rm micro}$.
	\end{tcolorbox}
	...
	\begin{equation}
		F(r) = \frac{GMm}{r^2}\!\left[1 + \frac{9}{2}\!\left(\frac{\lambda_C}{r}\right)^{\!2}
		      + \frac{7\cdot16}{360}\!\left(\frac{\lambda_C}{r}\right)^{\!4}
		      + \cdots\right].
	\end{equation}
	More precisely, each coefficient $a_k$ in
	$F = (GMm/r^2)\sum_{k=0}^\infty a_k(\lambda_C/r)^{2k}$
	...
	\end{equation}
	This is the \textbf{exact QGD quantum-gravity force law} valid for all
	$r\in(0,\,\pi\lambda_C/2)$.
	...
	\caption{First six quantum-gravity force coefficients.
	  $F(r) = (GMm/r^2)\sum_k a_k(2r/\lambda_C)^{2k}$ from~\eqref{eq:F_exact_closed}.}
	\begin{tabular}{@{}ccccc@{}}
	\toprule
	...
	\begin{itemize}
		...
		\item \textbf{Classical limit} $r\gg\lambda_C$: the Taylor expansion
		  is invalid (phase $t=2r/\lambda_C\gg1$); Newton's law is recovered
		  from the current-conservation bridge and the QGD field equation instead
