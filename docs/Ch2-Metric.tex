...
$\Gamma^r_{tt} = f\rs/(2r^2)$ acting on the covector index:
\begin{equation}
  \nabla_t\nabla_t\sigt = -\Gamma^r_{tt}\,\nabla_r\sigt
  = -\frac{f\rs}{2r^2}\cdot\frac{\sqrt{\rs}}{2\sqrt{r}(\rs-r)}
  = \frac{\rs^{3/2}}{4r^{7/2}(r-\rs)}\cdot\frac{r}{\sqrt{r}}\cdot\ldots
\end{equation}
and is absent in any \emph{scalar} formulation.  This term is essential for exact
closure: without it, the sum gives a residual $-\rs^{3/2}/(4r^{7/2}) \neq 0$.

Summing all four components and using $g^{tt} = -1/f$ gives
...
(see Remark~\ref{rem:grr}).  The physics is entirely encoded in $\sigma_t$; the
apparent spatial curvature in $g_{rr}$ is a coordinate artefact.
...
\begin{itemize}[nosep]
  ...
  \item The Schwarzschild $\sigma$-field satisfies $\boxg\sigt = \sigt(\sigt^2-1)/(4r^2)$
    ...
    1-form d'Alembertian in the exact Schwarzschild background.
...
\begin{itemize}[nosep]
  \item Explicit symbolic verification that the full action variation in the
    ...
    via the Palatini/connection channel (formal derivation of $G_t^{(\mathrm{Chr})}$
    from first principles).
  % ── CORRECTION: g_rr removed from open problems; replaced with spin ──────────
